...
class BetaIdentityStore(LocalIdentityStore):
    ...
    @staticmethod
    def tournament_update_ack_mode() -> str:
        ...
        """
        raw = os.environ.get("FIFA14_TOURNAMENT_UPDATE_ACK", "auto").strip().lower()
        if raw in {"echo", "legacy", "full"}:
            return "echo"
        if raw in {"minimal", "safe", "identity"}:
            return "minimal"
        return "auto"
